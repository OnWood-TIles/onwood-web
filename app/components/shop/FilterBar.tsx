...
type FilterGroupVM = { slug: string; label: string; activeCount: number; values: FilterVal[] };
...
// Full-width horizontal filter bar: one dropdown per group across the top of
// the product list. Values still navigate via shareable ?f= URLs (server-side
// filtering); only open/close is client state.
export function FilterBar({ groups, clearAllHref, hasActive }: { groups: FilterGroupVM[]; clearAllHref: string; hasActive: boolean }) {
  ...
  return (
    ...
    >
      ...
      {groups.map((g) => {
        ...
        const activeHere = g.activeCount > 0;
        return (
          <div key={g.slug} style={{ position: "relative" }}>
            <button
              ...
              style={{
                ...
                color: activeHere ? "var(--accent)" : "var(--ink)",
                transition: "all .2s ease",
              }}
            >
              {g.label}
              {activeHere ? ` (${g.activeCount})` : ""}
                ...
                {g.values.map((v) => (
                  <Link
                    ...
                    style={{
                      ...
                      color: v.active ? "var(--accent)" : "var(--ink)",
                      background: v.active ? "color-mix(in oklab, var(--accent) 10%, transparent)" : "transparent",
                    }}
                  >
                    {v.hex && (
                      <span style={{ width: 14, height: 14, borderRadius: "50%", background: v.hex, border: "1px solid rgba(0,0,0,.15)", flexShrink: 0 }} />
                    )}
      ...
      {hasActive && (
        <Link href={clearAllHref} style={{ marginLeft: "auto", fontSize: 12.5, fontWeight: 700, color: "var(--accent)", textDecoration: "none", padding: "0 6px" }}>
          Clear all ✕
        </Link>
      )}
    </div>
